...
public class JdbcHelperTest extends JDBCTestSupport {
    ...
    public void testMeteoPrimaryKeys() throws Exception {
        ...
        SortedMap<EntityMetadata, JdbcPrimaryKeyConstraintMetadata> pkMap = JDBC_HELPER.getPrimaryKeyColumns(metaData, tables);
    ...
    public void testMeteoStationsTablePrimaryKey() throws Exception{
    	DatabaseMetaData metaData = this.setup.getDataSource().getConnection().getMetaData();    	
        JdbcTableMetadata table = new JdbcTableMetadata(metaData.getConnection(), null, SCHEMA, "meteo_stations");
    ...
    public void testMeteoStationsTableColumns() throws Exception{
        ...
        List<JdbcTableMetadata> tables = new ArrayList<JdbcTableMetadata>();
        tables.add(table);
        SortedMap<JdbcTableMetadata, List<AttributeMetadata>> cMap = JDBC_HELPER.getColumns(metaData, tables);
    ...
    public void testMeteoObservationsTableColumns() throws Exception{
    	DatabaseMetaData metaData = this.setup.getDataSource().getConnection().getMetaData();    	
        JdbcTableMetadata table = new JdbcTableMetadata(metaData.getConnection(), null, SCHEMA, "meteo_observations");
        List<JdbcTableMetadata> tables = new ArrayList<JdbcTableMetadata>();
        tables.add(table);
        SortedMap<JdbcTableMetadata, List<AttributeMetadata>> cMap = JDBC_HELPER.getColumns(metaData, tables);
        SmartAppSchemaTestHelper.printColumns(cMap);
    ...
    public void testMeteoSchemaForeignKeys() throws Exception {
    	DatabaseMetaData metaData = this.setup.getDataSource().getConnection().getMetaData();    	
        List<JdbcTableMetadata> tables = JDBC_HELPER.getSchemaTables(metaData, SCHEMA);
        SortedMap<JdbcForeignKeyConstraintMetadata, Collection<JdbcForeignKeyColumnMetadata>> fkMap = JDBC_HELPER.getForeignKeys(metaData, tables);
        SortedMap<EntityMetadata, JdbcPrimaryKeyConstraintMetadata> pkMap = JDBC_HELPER.getPrimaryKeyColumns(metaData, tables);
        SortedMap<String, Collection<String>> uniqueIndexMultimap = JDBC_HELPER.getIndexColumns(metaData, tables, true, true);
    ...
    public void testMeteoObservationsTableForeignKeys() throws Exception {
    	DatabaseMetaData metaData = this.setup.getDataSource().getConnection().getMetaData();
        JdbcTableMetadata table = new JdbcTableMetadata(metaData.getConnection(),null, SCHEMA, "meteo_observations");
        SortedMap<JdbcForeignKeyConstraintMetadata, Collection<JdbcForeignKeyColumnMetadata>> fkMultimap = JDBC_HELPER.getForeignKeysByTable(metaData, table);
        JdbcPrimaryKeyConstraintMetadata primaryKey = JDBC_HELPER.getPrimaryKeyColumnsByTable(metaData, table);
        SortedMap<EntityMetadata, JdbcPrimaryKeyConstraintMetadata> pkMap = new TreeMap<EntityMetadata, JdbcPrimaryKeyConstraintMetadata>();
        pkMap.put(table, primaryKey);
        SortedMap<String, Collection<String>> uniqueIndexMultimap = JDBC_HELPER.getIndexesByTable(metaData, table, true, true);
    ...
    public void testIndexes() throws Exception {
    	DatabaseMetaData metaData = this.setup.getDataSource().getConnection().getMetaData();
        List<JdbcTableMetadata> tables = JDBC_HELPER.getSchemaTables(metaData, SCHEMA);
        SortedMap<String, Collection<String>> uniqueIndexMultimap = JDBC_HELPER.getIndexColumns(metaData, tables, false, true);
    ...
    public void testMeteoStationTableIndexes() throws Exception {
    	DatabaseMetaData metaData = this.setup.getDataSource().getConnection().getMetaData();
        JdbcTableMetadata table = new JdbcTableMetadata(metaData.getConnection(),null, SCHEMA, "meteo_stations");
        SortedMap<String, Collection<String>> uniqueIndexMultimap = JDBC_HELPER.getIndexesByTable(metaData, table, false, true);
        SmartAppSchemaTestHelper.printIndexes(uniqueIndexMultimap);
    	metaData.getConnection().close();
    }
